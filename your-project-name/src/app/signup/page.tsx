...
export default function Signup() {
  ...
  const handleSignup = (e: FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
    console.log("Signup sequence initiated");
  };
          ...
          <div className="relative z-10 flex flex-col h-full justify-between gap-8 md:gap-12 text-white">
            <div className="space-y-6 md:space-y-10">
              ...
              <div className="space-y-3 md:space-y-4">
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif leading-tight md:leading-[1.1] tracking-tight">
                  ...
                  <span className="italic text-[#D4AF37] font-light">for the frontier.</span>
                </h2>
                
                <p className="text-base md:text-lg text-white/50 max-w-sm font-medium leading-relaxed">
                  Join a global network of practitioners bridging the gap between rural clinics and modern clinical infrastructure.
                </p>
              </div>
              ...
              <div className="hidden sm:flex flex-col gap-3 md:gap-4">
                <div className="flex items-center gap-4 px-4 py-3 md:px-5 md:py-4 rounded-xl md:rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl">
                  ...
                  <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] text-white/80">ISO 27001 Data Guard</span>
                </div>
                <div className="flex items-center gap-4 px-4 py-3 md:px-5 md:py-4 rounded-xl md:rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl">
                  <Globe className="w-4 h-4 md:w-5 md:h-5 text-[#D4AF37]" />
                  <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] text-white/80">Cross-Clinic Interop</span>
                </div>
              </div>
            </div>
            ...
            <div className="hidden md:block pt-8 border-t border-white/10">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">
                © 2024 Clinical Sovereignty Protocol
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-md mx-auto w-full">
            <header className="mb-8 md:mb-12 space-y-3 md:space-y-4">
              ...
              <p className="text-sm md:text-base text-slate-500 font-medium">
                Enter your credentials to join the interoperable network.
              </p>
            </header>
            ...
            <form className="space-y-5 md:space-y-6" onSubmit={handleSignup}>
              ...
              {/* Security Key */}
              <div className={`space-y-1 md:space-y-2 transition-all duration-700 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                <label className="text-[9px] md:text-[10px] font-black text-[#1A2E26]/60 uppercase tracking-widest ml-1">
                  Security Key
                </label>
                <div className="relative group">
              ...
              {/* Compliance Check */}
              <div className={`flex items-start gap-3 pt-1 md:pt-2 transition-all duration-700 delay-400 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
                <div className="relative flex items-center h-5 mt-0.5">
                  <input 
                    id="compliance" 
                    type="checkbox" 
                    required
                    className="w-4 h-4 md:w-5 md:h-5 rounded md:rounded-lg border-2 border-[#F4F6F2] text-[#1A2E26] focus:ring-[#D4AF37] cursor-pointer" 
                  />
                </div>
                <label htmlFor="compliance" className="text-[11px] md:text-xs font-medium text-slate-500 cursor-pointer select-none leading-relaxed">
                  I agree to the <span className="text-[#1A2E26] underline underline-offset-2">HIPAA Data Processing</span> protocols and the Clinical Sovereignty Terms.
                </label>
              </div>
              ...
              <button 
                type="submit"
                className={`w-full bg-[#1A2E26] hover:bg-[#254136] text-white font-black py-4 md:py-5 rounded-xl md:rounded-2xl flex items-center justify-center gap-3 shadow-xl shadow-[#1A2E26]/10 active:scale-[0.99] transition-all duration-500 delay-500 group uppercase text-[10px] md:text-[11px] tracking-[0.2em] ${
            ...
            <footer className={`mt-8 md:mt-10 pt-6 md:pt-8 border-t border-slate-100 transition-all duration-700 delay-600 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
              <div className="flex flex-col items-center gap-4">
                <p className="text-slate-500 text-xs md:text-sm font-medium text-center">
                  Member of the network?{" "}
                  <button onClick={() => goToLogin()} className="text-[#1A2E26] font-black hover:text-[#D4AF37] transition-colors uppercase text-[9px] md:text-[10px] tracking-widest block sm:inline mt-2 sm:mt-0">
                    Login here
                  </button>
                </p>
